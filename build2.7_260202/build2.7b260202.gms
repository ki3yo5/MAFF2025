...
$if not setglobal ncf    $setglobal ncf    0
...
$if not setglobal dsrt   $setglobal dsrt   0
...
$if not setglobal fertrep  $setglobal fertrep  0
...
* name output files
$iftheni %KEY% == 1_0
$if not setglobal gdx_results   $setglobal gdx_results   .\results\%build%_data%dataYr%_import%imscn%_%name%_fbal_results.gdx
$if not setglobal excel_results $setglobal excel_results .\results\%build%_data%dataYr%_import%imscn%_%name%_fbal_results.xlsx
$elseif %KEY% == 0_1
$if not setglobal gdx_results   $setglobal gdx_results   .\results\%build%_data%dataYr%_import%imscn%_%name%_liebig_results.gdx
$if not setglobal excel_results $setglobal excel_results .\results\%build%_data%dataYr%_import%imscn%_%name%_liebig_results.xlsx
$else
$if not setglobal gdx_results   $setglobal gdx_results   .\results\%build%_data%dataYr%_import%imscn%_%name%_results.gdx
$if not setglobal excel_results $setglobal excel_results .\results\%build%_data%dataYr%_import%imscn%_%name%_results.xlsx
$endif
...
Scalar    
    ...
    weight     'Weight on calorie balance in the objective function' /  0.75  /
    limit      'Upper bound for area expansion and feed production'  /  3     /
    ;
...
* dairycow lactating to produce milk
...
* mind the error division by 0 for data(g,"total")
...
Equation
    ...
    jan                  'Cropping area may not current area in the first month (1000ha)'
    feb                  'Each month of commencement of import shock, already cropped area cannot be changed'
    mar                  'Simulation scenario varies in twelve ways, when the import shock begins'
    apr
    may
    jun
    jul
    ...
    elesupply(e)..        eles(e)  =e= esupply("prod",e) + ncfcoef*esupply("import",e) + esupply("organic",e);
...
*Residual land per month (croppable in "paddy_dry" or "field_irr" or "field_rain" and other region than "Hokkaido") 
    resLand(pattern,r,l,t) $((sameas(l,'paddy_dry') or sameas(l,'field_irr') or sameas(l,'field_rain')) and (not sameas(r,'Hokkaido'))) =
...
* Assign maximum area over t
    ...
    dietxveg(pattern)    = totalxveg(pattern)*x2n("veges")    /(tpop*(10**6)*365);
    nutxveg(pattern,nn)  = totalxveg(pattern)*x2nn("veges",nn)/(tpop*(10**6)*365);

display resLand, resLabor, xveg_resLand, xveg_resLabor, xveg, totalxveg;

*Assign "veges" values by post-calculation
    pxcrop(pattern,"veges") = totalxveg(pattern);
    pxdpotential_c(pattern,"veges") = dietxveg(pattern);
    pxnnpotential_c(pattern,"veges",nn) = nutxveg(pattern,nn);
...
*Assign "veges" values by post-calculation
    plandrep_region(pattern,"total","veges") = totalxveg(pattern);
    plandrep_month(pattern,t,"total","veges") = xveg(pattern,t);
