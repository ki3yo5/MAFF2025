...
* enable gdxxrw (only for developper):
...
* model arguments
    ...
    r region /
    cold          'Hokkaido'
    temperate     'Honsyu, Shikoku and Kyusyu'
    subtropical   'Okinawa'/
    l land type /
    paddy         'paddy field'
    convertible   'convertible paddy field'
    field         'upland field'
    orchard       'orchard'
    pasture       'pasuture'/
...
parameter
    ...
    landreq(c,t)      'Seasonal requirement by crop (0/1)'
...
Table
    land(r,l) 'Land endowments (1000ha)'
                    paddy   convertible   field   orchard   pasture
    cold            209.8         0       418.0      3.1     498.1
    temperate       886.3      1110       674.2    248.5      84.9
    subtropical       0.7         0        27.5      1.9       6.0 ;
...
$ifi %liebig%==1                                          *zyield.l(i)/data(i,"yield")
    ...
    wlandrep_month(w,t,l,c) = sum(r, wxrlcrop(w,c,r,l)*landreq(c,t));
...
$ifi %liebig%==1                                          *zyield.l(i)/data(i,"yield")
    ...
    llandrep_month(u,t,l,c)       = sum(r,lxrlcrop(u,c,r,l)*landreq(c,t));
...
parameters
    resUpland(pattern,r,t)    'Residual upland by month       (1000ha)'
    resSecond(pattern,r,t)    'Residual upland by month       (1000ha)'
    resLabor(pattern)         'Residual labor                 (bil. hour)'
    xveg_resLand(pattern,r,t) 'Maximum cropping area with residual land  (1000ha)'
    xveg_resLabor(pattern)    'Maximum cropping area with residual labor (1000ha)'
    xveg(pattern,r,t)         'Post-calculation cropping area (1000ha)'
    totalxveg(pattern,r)      'Annual cropping area (1000ha)'
;
scalar
...
*Residual land and labor per month
    resUpland(pattern,r,t) = land(r,"field") - sum(c, pxrlcrop(pattern,c,r,"field") * landreq(c,t));
    resSecond(pattern,r,t) = land(r,"convertible") - sum(c, pxrlcrop(pattern,c,r,"convertible") * landreq(c,t));
    resLabor(pattern)    = labor   - sum(c, pxcrop(pattern,c) * ldemand_c(c))   /(10**9)
                                   - sum(ls, pxlive(pattern,ls)* ldemand_ls(ls))/(10**9);

* Non-negativity check
    resUpland(pattern,r,t) = max(0, resUpland(pattern,r,t));
    resSecond(pattern,r,t) = max(0, resSecond(pattern,r,t));
    resLabor(pattern)    = max(0, resLabor(pattern));

* Maximum cropping area with either residual input
    xveg_resLand(pattern,r,t) = resUpland(pattern,r,t)+resSecond(pattern,r,t);
    xveg_resLabor(pattern)  = resLabor(pattern)/laborVeg;

* Joint constraint on cropping area of "mis_veges"
    xveg(pattern,r,t) = min( xveg_resLand(pattern,r,t), xveg_resLabor(pattern) );
    totalxveg(pattern,r) = smax(t, xveg(pattern,r,t));


display resUpland, resSecond, resLabor, xveg_resLand, xveg_resLabor, xveg, totalxveg;

* Assign "mis_veges" as maximum area over t
    pxcrop(pattern,"mis_veges") = sum(r, totalxveg(pattern,r));
...
$ifi %liebig%==1                                                      *zyield.l(i)/data(i,"yield")
    ...
    plandrep_region(pattern,r,"mis_veges") = totalxveg(pattern,r);
    ...
    plandrep_month(pattern,t,l,c) = sum(r, pxrlcrop(pattern,c,r,l)*landreq(c,t));
    plandrep_month(pattern,t,"field","mis_veges") = sum(r, xveg(pattern,r,t));
...
*Restore changed parameters
    land("temperate","field") = 674.2;
    land("temperate","pasture") = 84.9;
    land("subtropical","field") = 27.5;
    land("subtropical","pasture") = 6.0;
...
Execute_Unload '%gdx_results%',
    ...
    wlandrep_region, wlandrep_month,
    llandrep_region, llandrep_month,
    plandrep_region, plandrep_month,
    fertrep, fertrep_g,
...
execute 'gdxxrw %gdx_results% o=%excel_results% par=plandrep_month  rng=plandrep!A12 rdim=2 cdim=2'
$endif
$endif
