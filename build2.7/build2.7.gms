...
$if not setglobal build   $setglobal build 2.7
...
parameters
    laborVeg(t)             'mis_veges labor demand per month (bil. hour /1000ha)'
    resUpland(pattern,t)    'Residual upland by month       (1000ha)'
    resSecond(pattern,t)    'Residual upland by month       (1000ha)'
    resLabor(pattern,t)     'Residual labor                 (bil. hour)'
    xveg(pattern,t)         'Post-calculation cropping area (1000ha)'
    totalxveg(pattern)      'Average cropping area            (1000ha)'
;

*Assumed monthly input and yield as 1/3 (assumed to require 3 months for prouction) 
    laborVeg(t) = ldemand_c("mis_veges")   /(3*10**9);
    ...
    resLabor(pattern,t)  = labor/12  - sum(c, pxcrop(pattern,c) * landreq(c,t) * ldemand_c(c))/(10**9);

* Non-negativity check
    resUpland(pattern,t) = max(0, resUpland(pattern,t));
    resSecond(pattern,t) = max(0, resSecond(pattern,t));
    resLabor(pattern,t)  = max(0, resLabor(pattern,t));

* Joint constraint on cropping area of "mis_veges"
    xveg(pattern,t) = min( (resUpland(pattern,t)+resSecond(pattern,t)), resLabor(pattern,t)/laborVeg(t) );
    totalxveg(pattern) = sum(t, xveg(pattern,t))/12;


display resUpland, resSecond, resLabor, xveg, totalxveg;
...
execute 'gdxxrw %gdx_results% o=%excel_results% par=paradar  rng=pradar!A45  rdim=2 cdim=1'
$endif
...
execute 'gdxxrw %gdx_results% o=%excel_results% par=plaborrep_g rng=plaborrep!A9 rdim=1 cdim=1'
$endif
$endif
