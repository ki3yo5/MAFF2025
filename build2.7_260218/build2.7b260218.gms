...
Simultaneous simulation for cropping and animal production models with 18 crops, 6 processing foods,
18 feeds, 7 livestocks, 5 animal products and 2 marine products.
The objective function consists of calorie deficit and net food intake balance of 10 food groups.
...
$if not setglobal ncf    $setglobal cfim   0
...
* adjust processing rate (oil) of domestically produced soybeans
    ...
    x2f("oil","rice") = data("rice","yield")*pcoef("oil","rice")*(10**9);    
...
Display CRT, esupply, ldemand_c, land_endw, acoef , tpop;
...
* Assign calculated values
    ...
    wmarinerep               'Marine product summary'
...
*Assign "fish" "seaweed" values as constant (Fish production to TAC limit 4650(1000MT) )
    pxdpotential_c(pattern,"fish")       = nnpc("fish")*4650*g2n("fish")/data("fish","net");
    pxnnpotential_c(pattern,"fish",nn)   = nnpcvalue("fish",nn)*4650*g2n("fish")/data("fish","net");
    pxdpotential_c(pattern,"seaweed")    = nnpc("seaweed");
    pxnnpotential_c(pattern,"seaweed",nn)= nnpcvalue("seaweed",nn);    
    ...
    pmarinerep(pattern,c,"current_cal")    = nnpcvalue(c,"calorie") $marine(c);
    pmarinerep(pattern,c,"potential_cal")  = pxnnpotential_c(pattern,c,"calorie") $marine(c); 
    pmarinerep(pattern,c,"current_diet")   = nnpc(c) $marine(c);
    pmarinerep(pattern,c,"potential_diet") = pxdpotential_c(pattern,c) $marine(c);
    pmarinerep(pattern,c,"import_diet")    = impc(c) $marine(c);
    pmarinerep(pattern,c,"change_diet")    = pmarinerep(pattern,c,"potential_diet")+pmarinerep(pattern,c,"import_diet")-pmarinerep(pattern,c,"current_diet");
    pmarinerep(pattern,"total",frep)       = sum(c, pmarinerep(pattern,c,frep));

    pdietrep(pattern,nn,"required")      = nreq(nn);
    pdietrep(pattern,nn,"current")       = sum(g, nnpcvalue(g,nn));
    pdietrep(pattern,nn,"potential")     = 
                                           + sum(ap, pxnnpotential_l(pattern,ap,nn))
                                           + sum(c,  pxnnpotential_c(pattern,c,nn) $marine(c))
                                           + sum(c,  pxnnpotential_c(pattern,c,nn)*npecoef(c) $edible(c))
                                           + sum(c,  pxnnpotential_f(pattern,c,nn)*npecoef(c) $processed(c));
...
Execute_Unload '%gdx_results%',
    ...
    pcroprep, pfoodrep, pliverep, pmarinerep, pdietrep, pradar, paradar, plandrep_region, plandrep_month, plaborrep, pfertrep,
...
execute 'gdxxrw %gdx_results% o=%excel_results% par=pmarinerep rng=pmarinerep!A1 rdim=2 cdim=1'
