...
Display landreq_crt, mmap, rmap, lmap, CRL, esupply, ldemand_c;
...
Table
    land(r,l)   'Land endowments (1000ha)'
                  paddy_wet   paddy_dry   field_rain   field_irr   orchard   pasture
    Hokkaido        70.2        151.3        190.9       227.1       3.1      498.1
    Tohoku         301.5        285.3        106.4        20.3      44.5       55.8
    Kanto_west     847.4        678.5        327.1       220.4     204.2       29.1
    Okinawa          0.3          0.4          5.1        22.4        1.9       6.0 ;
...
$ifi %lbal%==1                labdemand_c,labdemand_l,labbal
    ...
    Model animal_constraint /
                            ...
                            /;
    Model balance           / nutpet_c, nutpet_f, nutpet_l,  caloriebal,caloriebal_r
                              dietpet_c,dietpet_f,dietpet_l, dietbal,   dietbal_r,   adietbal,adietbal_r
                            /;
* Standard solution
...
*Convert pasture to field_rain except Hokkaido
    land(r,"field_rain") $(not sameas(r,'Hokkaido')) = land(r,"field_rain") + land(r,"pasture");
    land(r,"pasture") $(not sameas(r,'Hokkaido')) = 0;
...
*Residual land (croppable in "paddy_dry" or "field_irr") and labor per month
    resLand(pattern,r,l,t) $(sameas(l,'paddy_dry') or sameas(l,'field_irr')) = land(r,l) - sum(c, pxrlcrop(pattern,c,r,l)*landreq(c,t));
...
*                                           *x2nn(c,"calorie") $edible(c);
    ...
    pcroprep(pattern,c,"potential_diet") = pxdpotential_c(pattern,c)$edible(c);
    pcroprep(pattern,c,"current_cal")    = nnpcvalue(c,"calorie")$edible(c);
    pcroprep(pattern,c,"potential_cal")  = pxnnpotential_c(pattern,c,"calorie")$edible(c);
    pcroprep(pattern,c,"import_diet")    = impc(c)$edible(c);
    pcroprep(pattern,c,"change_diet")    = pcroprep(pattern,c,"potential")+pcroprep(pattern,c,"import")-pcroprep(pattern,c,"current");
...
*                                                 *x2fn(c,i,"calorie") $processed(c));
    ...
    pfoodrep(pattern,c,"potential_diet") = pxdpotential_f(pattern,c)$processed(c);
    pfoodrep(pattern,c,"potential_cal")  = pxnnpotential_f(pattern,c,"calorie")$processed(c);
